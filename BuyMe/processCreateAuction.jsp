...
<%
    ...
    // Build full description with vehicle details
    String fullDescription = String.format("Make: %s | Model: %s | Year: %s | Mileage: %s | Color: %s | %s", 
                                          make, model, year, mileage, color, description);
    ...
    try {
        ...
        // Insert item
        pst = con.prepareStatement("INSERT INTO item (item_id, title, item_description, item_condition, date_posted, sub_category_id) VALUES (?, ?, ?, ?, CURDATE(), ?)");
        pst.setInt(1, itemId);
        pst.setString(2, title);
        pst.setString(3, fullDescription);
