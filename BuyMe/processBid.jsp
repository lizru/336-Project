...
<%
    ...
    try {
        ...
        // ALERT SYSTEM: Notify all previous bidders that they've been outbid (manual alert)
        pst = con.prepareStatement(
            "SELECT DISTINCT b.user_id " +
            "FROM bid b " +
            "WHERE b.auction_id = ? AND b.item_id = ? AND b.user_id != ?"
        );
        pst.setInt(1, auctionId);
        pst.setInt(2, itemId);
        pst.setInt(3, userId);
        ...
        while (rs.next()) {
            int outbidUserId = rs.getInt("user_id");
            
            PreparedStatement alertPst = con.prepareStatement(
                "INSERT INTO bid_alert (user_id, auction_id, item_id, alert_type, message, new_bid_amount, date_created, is_read) " +
                "VALUES (?, ?, ?, 'outbid', ?, ?, NOW(), false)"
            );
            alertPst.setInt(1, outbidUserId);
            alertPst.setInt(2, auctionId);
            alertPst.setInt(3, itemId);
            alertPst.setString(4, "You have been outbid! A new bid of $" + String.format("%.2f", bidAmount) + " has been placed.");
            alertPst.setDouble(5, bidAmount);
            alertPst.executeUpdate();
            alertPst.close();
        }
        rs.close();
        pst.close();
        ...
        if (!isAutoBid) {
            ...
            while (rs.next()) {
                ...
                if (newAutoBidAmount <= autoBidLimit) {
                    ...
                    // ALERT: Autobid limit exceeded (automatic alert)
                    PreparedStatement alertPst = con.prepareStatement(
                        "INSERT INTO bid_alert (user_id, auction_id, item_id, alert_type, message, new_bid_amount, date_created, is_read) " +
                        "VALUES (?, ?, ?, 'limit_exceeded', ?, ?, NOW(), false)"
                    );
                    alertPst.setInt(1, autoBidUserId);
                    alertPst.setInt(2, auctionId);
                    alertPst.setInt(3, itemId);
                    alertPst.setString(4, "Your autobid limit of $" + String.format("%.2f", autoBidLimit) + " has been exceeded. Current bid: $" + String.format("%.2f", bidAmount));
                    alertPst.setDouble(5, bidAmount);
                    alertPst.executeUpdate();
                    alertPst.close();
                }
